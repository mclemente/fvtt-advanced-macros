"use strict";const MODE_NONE='modeNone';const MODE_DEFAULT='modeDefault';const MODE_MATCH='modeMatch';const sortTokenizableSubstrings=(a,b)=>{if(a.length>b.length){return-1}
if(a.length<b.length){return 1}
return 0};const endsWith=(str,suffix)=>{return str.indexOf(suffix,str.length-suffix.length)!==-1};class Tokenizer{constructor(factory,str,forEachToken){this.factory=factory;this.str=str;this.forEachToken=forEachToken;this.previousChr='';this.toMatch='';this.currentToken='';this.modeStack=[MODE_NONE];this.currentIndex=0}
getCurrentMode(){return this.modeStack[this.modeStack.length-1]}
setCurrentMode(mode){return this.modeStack.push(mode)}
completeCurrentMode(){const currentMode=this.getCurrentMode();if(currentMode===MODE_DEFAULT){this.pushDefaultModeTokenizables()}
if((currentMode===MODE_MATCH&&this.currentToken==='')||this.currentToken!==''){this.push(this.currentToken)}
this.currentToken='';return this.modeStack.pop()}
push(token){let surroundedBy='';if(this.factory.convertLiterals&&this.getCurrentMode()!==MODE_MATCH){switch(token.toLowerCase()){case 'null':token=null;break;case 'true':token=!0;break;case 'false':token=!1;break;default:if(isFinite(token)){token=Number(token)}
...
while(this.getCurrentMode()!==MODE_NONE){this.completeCurrentMode()}}
consume(chr){this[this.getCurrentMode()](chr);this.previousChr=chr}[MODE_NONE](chr){if(!this.factory.matchMap[chr]){this.setCurrentMode(MODE_DEFAULT);return this.consume(chr)}
this.setCurrentMode(MODE_MATCH);this.toMatch=chr}[MODE_DEFAULT](chr){if(this.factory.delimiterMap[chr]){return this.completeCurrentMode()}
...
if(lowestIndexOfTokenize!==-1){this.push(toTokenize);this.currentToken=this.currentToken.substring(lowestIndexOfTokenize+toTokenize.length);return this.pushDefaultModeTokenizables()}}[MODE_MATCH](chr){if(chr===this.toMatch){if(this.previousChr!==this.factory.escapeCharacter){return this.completeCurrentMode()}
